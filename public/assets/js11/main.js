...
function spin() {
  ...
  setTimeout(() => {
    ...
    fetch(`/coupon/code_coupon/${value}`)
      ...
      .then((data) => {
        // get coupon type from response data
        const couponType = data.couponType;

        if (couponType === 'simple') {
          // generate qrcode with code_coupon
          const qrcode = generateQrcode(value, data.code_coupon);
          ...
          Swal.fire({
            icon: "success",
            title: "Co!",
            html: `You won ${value}! Your coupon code is: <strong>${data.code_coupon}${showQrCode ? `  Click <a href="${qrcode}" target="_blank">here</a> to view your QR code.</strong>.` : '</strong>'}`,
          ...
          // show error message for non-simple coupon types
          Swal.fire({
            icon: "error",
            title: "Oops...",
            text: "This coupon type is not eligible for a QR code.",
          }).then((result) => {
            if (result.isConfirmed) {
              spin();
            }
